...
- **Každý HTTP provider skládal autentizaci jinak, takže výměna HTTP klienta měnila chování přihlášení.** `SymfonyClientProvider` posílal u typu přihlášení `HOSTED_SPIS` hlavičku `Basic base64("dbID")` bez dvojtečky (dle RFC 7617 nevalidní; Guzzle správně posílá `base64("dbID:")`) a při chybějícím `dbID`, přihlašovacím jménu nebo hesle request stejně odeslal — s `Basic base64("")`, resp. `Basic base64(":")` — a zbytečně čerpal limit neúspěšných pokusů o přihlášení. Skládání hlaviček i pověření je nově v jednom sdíleném `Provider\ClientRequestTrait` (včetně kontrol na chybějící údaje, které vyhodí `MissingRequiredField` ještě před odesláním), takže oba providery posílají bajt po bajtu stejné hlavičky. Symfony provider navíc používá nativní volbu `auth_basic` místo ručního `base64_encode`.
