...
class DataBox extends Connector
{
    ...
    {
        return $this->send($account, self::ACCESS, (new Request\GetOwnerInfoFromLogin()), Response\GetOwnerInfoFromLogin::class);
    }

    public function changeIsdsPassword(Account $account, Request\ChangeISDSPassword $input): Response\ChangeISDSPassword
    {
        return $this->send($account, self::ACCESS, $input, Response\ChangeISDSPassword::class);
    }

    public function getPasswordExpirationInfo(Account $account): Response\GetPasswordInfo
    {
        return $this->send($account, self::ACCESS, (new Request\GetPasswordInfo()), Response\GetPasswordInfo::class);
    }
